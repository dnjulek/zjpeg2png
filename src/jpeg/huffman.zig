...
//! Source: src/formats/jpeg/huffman.zig — t-81 section 2.4.2 huffman tables
//! and the entropy-coded-segment bit reader.
    ...
    pub fn read(allocator: std.mem.Allocator, table_class: u8, reader: *std.Io.Reader) Image.ReadError!Table {
        ...
        for (code_counts, 0..) |count, i| {
            ...
            while (j < count) : (j += 1) {
                ...
                // construct acceleration structure, see stb_image
                if (i + 1 <= fast_bits) {
